...
| AI | Genkit (Go SDK), system prompt 模板, Gemini 3 Flash (preview → GA) |
...
- `internal/notion/`（Notion page reader，Phase 3）
...
| ~~**C6 回顧提醒**~~ | ~~Cron + 模板文字~~ → Notion Weekly Review Agent 取代（TDR #17） | ~~3~~ |
...
-- notion_sync_state（簡化：Notion Agent 預處理後只需 last-read timestamp）
notion_sync_state (page_id TEXT PRIMARY KEY, last_modified_at TIMESTAMPTZ)
```
...
| notion-sync | C1, C2, C5 | 無 LLM | Cron（讀 Notion Agent output pages） |
...
- [ ] `internal/notion/` — Notion page reader（讀 Notion Agent 預處理的固定 page + database entry）
- [ ] `notion-sync` flow — C1 Projects 單向同步
- [ ] C2 Tasks 追蹤（讀 Notion Agent pre-processed pages，不需 AI）
- [ ] C5 Reading List → 觸發 B3 bookmark 生成
- ~~C6 週期回顧提醒~~ → 由 Notion Weekly Review Agent 取代（見 TDR #17）
...
- [ ] `planning` flow — I1 Morning Brief（input 包含 Notion Morning Brief Agent output page）、I3 週五回顧（input 包含 Notion Weekly Review Agent output + GitHub commits + RSS highlights）、G2 系列文規劃（Content Planner Agent 建議作為 input source，pipeline 做 cross-reference validation）
- [ ] G1 內容策略建議
...
| 17 | Notion Custom Agent 整合 | 自建 Notion polling vs Notion Agent → pipeline 讀取 | **Notion Agent → 固定 page → pipeline 讀取** | Notion Custom Agent 做 Notion 側預處理（Morning Brief、Weekly Review、Content Planner），pipeline 只讀 output page。單向資料流，MCP server 無 trigger 能力，不做反向觸發。Morning Brief 每天 07:30 → 固定 page，Weekly Review 每週五 17:00 → Weekly Reviews DB，Content Planner @mention 觸發。5 月前免費試用，5 月前決定付費保留哪些。 |
...
### Notion Agent Credits（5 月後計費）

| Agent | 頻率 | 預估 credits/月 |
|-------|------|----------------|
| Morning Brief | 每天 1 次 | ~300 |
| Weekly Review | 每週 1 次 | ~100 |
| Content Planner | 不定期 @mention | ~100-400 |
| **合計** | | **500-800 credits/月，$5-8** |

5 月前免費試用期，全部啟用觀察品質。5 月前決定付費保留哪些。
...
*最後更新：2026-03-10*
*v3.2：Notion Custom Agent 整合策略（TDR #17）、C6 砍掉、gemini-3-flash-preview（TDR #15）、proofread 4096 tokens（TDR #16）、Phase 1 進度更新*
