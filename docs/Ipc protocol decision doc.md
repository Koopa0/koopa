...
```sql
-- Backfill: 舊 directive 補 default routing（歷史紀錄，不影響 routing 邏輯）
UPDATE session_notes
SET target = 'hq', priority = 'p2'
WHERE note_type = 'directive' AND target IS NULL;

-- 完整 constraint（不需要 NOT VALID）
...
- 寫 report（有 `in_response_to`）：server 從 referenced directive 自動複製，覆蓋 agent 值
...
Go 層 validate：`target` 和 `acknowledged_by` 只接受 `role = 'department'`。`source` 兩種都接受（保持現狀相容）。FK 暫時指向整個 `actor` 表，不在 DB 層強制 role 區分——避免 partial unique index 或拆表的複雜度。
...
-- Step 1: Drop redundant source CHECK (before adding FK)
...
-- Step 3: Backfill existing directives (before adding constraint)
...
-- Step 5: Domain validation (完整 constraint，不需要 NOT VALID)
ALTER TABLE session_notes ADD CONSTRAINT chk_priority_values
  CHECK (priority IS NULL OR priority IN ('p0', 'p1', 'p2'));

-- Step 6: Structural enforcement — directive only (完整，不需要 NOT VALID)
ALTER TABLE session_notes ADD CONSTRAINT chk_directive_fields
  CHECK (note_type <> 'directive' OR (target IS NOT NULL AND priority IS NOT NULL));

-- Step 7: Paired constraint for acknowledgment (完整，不需要 NOT VALID)
...
if target != "" {
    actor, err := store.Actor(ctx, target)
    if err != nil { return fmt.Errorf("unknown target actor: %s", target) }
