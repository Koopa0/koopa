-- koopa0.dev 現有 PostgreSQL Schema (Baseline)
-- Status: 目前運行中，Contents table 0 筆資料
-- 用途: Phase 1 migration 的 baseline
...
('https://go.dev/blog/feed.atom', 'The Go Blog', 'daily', '{"go"}', '{}'),
('https://golangweekly.com/rss/', 'Golang Weekly', 'weekly', '{"go"}',
 '{"deny_title_patterns":["(?i)sponsored"]}'),
('https://www.alexedwards.net/blog/feed', 'Alex Edwards', 'daily', '{"go"}', '{}'),
('https://blog.rust-lang.org/feed.xml', 'Rust Blog', 'daily', '{"rust"}', '{}'),
('https://this-week-in-rust.org/atom.xml', 'This Week in Rust', 'weekly', '{"rust"}', '{}'),
('https://blog.angular.dev/feed', 'Angular Blog', 'daily', '{"angular","frontend"}', '{}'),
('https://blog.flutter.dev/feed', 'Flutter Blog', 'daily', '{"flutter","dart","mobile"}', '{}'),
('https://blog.cloudflare.com/rss/', 'Cloudflare Blog', 'daily', '{"infra","networking","workers"}',
...
('https://simonwillison.net/atom/everything/', 'Simon Willison''s Weblog', 'daily', '{"ai","llm"}', '{}'),
('https://research.google/blog/rss/', 'Google Research Blog', 'daily', '{"ai","ml"}',
 '{"deny_title_patterns":["(?i)health","(?i)medical","(?i)quantum","(?i)biology","(?i)climate","(?i)flood","(?i)wildfire"]}'),
('https://www.latent.space/feed', 'Latent Space', 'weekly', '{"ai","llm"}', '{}'),
('https://www.anthropic.com/blog/rss', 'Anthropic Blog', 'daily', '{"ai","llm","claude"}',
 '{"deny_title_patterns":["(?i)career","(?i)hiring","(?i)join our team"],"deny_tags":["careers"]}'),
('https://huggingface.co/blog/feed.xml', 'Hugging Face Blog', 'daily', '{"ai","llm","ml"}',
 '{"deny_title_patterns":["(?i)community update","(?i)partnership"],"deny_tags":["community","partnerships"]}'),
('https://blog.bytebytego.com/feed', 'ByteByteGo', 'weekly', '{"system-design"}',
...
CREATE INDEX IF NOT EXISTS idx_contents_published_at_pub
    ON contents (published_at DESC NULLS LAST) WHERE status = 'published';
CREATE INDEX IF NOT EXISTS idx_contents_source_obsidian
    ON contents (source_type) WHERE source_type = 'obsidian';
CREATE INDEX IF NOT EXISTS idx_flow_runs_completed
    ON flow_runs (content_id, flow_name, ended_at DESC) WHERE status = 'completed';
