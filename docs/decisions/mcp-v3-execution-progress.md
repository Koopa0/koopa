...
- **MCP tools:** 49 → 29 (target ~11). W4/W5 changed the count by 0 (no MCP tool surface); W6 C1 removed `system_status` + `manage_feeds` (40 → 38), the propose/commit subsystem — 6 `propose_*` + `commit_proposal` (38 → 31), `goal_progress` + `track_hypothesis` (31 → 29), `advance_work` + `update_note_maturity` (29 → 27), `archive_learning_target` (27 → 26), then the `manage_plan` `update_plan` action (still 26 — manage_plan kept, now 5 actions). **W6 C1 COMPLETE.** Remaining: C2 (`learning_read` + remove the 4 learning-read tools), C3 (`agent_notes` + `brief` + morning/reflection/delta/weekly + content) → ~11 target.
