...
Notion Source Registry（`notion_sources` + Admin UI + property_map）。Events-only generic sync。Prometheus counter 決定 Obsidian Tier 2。Notion unique ID property 評估。Cloudflare Tunnel path-based routing。

### Phase 2

Genkit Audit P2（flow 拆分、prompt injection、token logging）。MCP Server hybrid search（三路 RRF）。Notion sync 遷移到 property_map。Spaced Repetition。Content Maturity。

### Phase 3+

Genkit Audit P3。統一週報/月報。Drift Detection。Session Reconstruction。Knowledge Graph。

### Phase 4

Learning Dashboard。Personal Changelog。Cross-Project Transfer。Retrospective Intelligence。
