...
/** Cap on the markdown a row preview will scan — keeps the strip regexes linear. */
const PREVIEW_SOURCE_LIMIT = 2000;

/**
 * One-line plain-text preview of a markdown capture body for the inbox row.
 * Strips the common markdown tokens an agent capture carries — fences, inline
 * code, headings, lists, links, emphasis — so the row shows readable text
 * instead of literal `**`/`` ` ``/`#` symbols; the full rendered body lives in
 * the clarify dialog. Underscore emphasis is only stripped at word boundaries
 * so `snake_case_id` survives (GFM intraword rule). Never returns an empty
 * string for non-empty input: an all-markdown body (e.g. an image with no alt)
 * falls back to its whitespace-collapsed raw text so the row keeps its line.
 */
export function plainPreview(md: string): string {
  if (!md) return '';
  // Bound the work before the backtracking link/image regexes run: this is a
  // one-line preview, and the body is agent/MCP-authored free text, so an
  // unbounded body could make the regex pipeline quadratic and freeze the tab.
  const source = md.length > PREVIEW_SOURCE_LIMIT ? md.slice(0, PREVIEW_SOURCE_LIMIT) : md;
  const stripped = source
    // fenced code: keep the inner text, drop the fences + language token
    .replace(/```[^\n]*\n?([\s\S]*?)```/g, '$1')
    // inline code
    .replace(/`([^`]+)`/g, '$1')
    // images: keep alt text (drops to nothing when alt is empty)
    .replace(/!\[([^\]]*)\]\([^)]*\)/g, '$1')
    // links: keep the visible text
    .replace(/\[([^\]]+)\]\([^)]*\)/g, '$1')
    // autolinks <https://…> / <mailto:…>
    .replace(/<((?:https?|mailto):[^>]+)>/g, '$1')
    // line-leading: headings, blockquotes, list markers, horizontal rules
    .replace(/^[ \t]*#{1,6}[ \t]+/gm, '')
    .replace(/^[ \t]*>[ \t]?/gm, '')
    .replace(/^[ \t]*(?:[-*+]|\d+[.)])[ \t]+/gm, '')
    .replace(/^[ \t]*(?:-{3,}|\*{3,}|_{3,})[ \t]*$/gm, '')
    // emphasis — matched delimiters only
    .replace(/\*\*([^*]+)\*\*/g, '$1')
    .replace(/\*([^*]+)\*/g, '$1')
    .replace(/~~([^~]+)~~/g, '$1')
    // underscore emphasis only at word boundaries (snake_case_id is preserved)
    .replace(/(^|\W)__([^_\n]+)__(?=\W|$)/g, '$1$2')
    .replace(/(^|\W)_([^_\n]+)_(?=\W|$)/g, '$1$2')
