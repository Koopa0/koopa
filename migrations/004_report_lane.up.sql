...
--       'open' and is therefore visible as unfulfilled work.
--
...
COMMENT ON TABLE research_assignments IS
    ...
    'unfulfilled and visible. Distinct from tasks (the A2A coordination entity, '
    'with acknowledge/file_report/revision lifecycle) — research assignments '
    'have no such lifecycle by design.';
...
COMMENT ON COLUMN research_assignments.status IS
    'Lifecycle: open → fulfilled. open = dispatched, no report yet (visible as '
    'unfulfilled work). fulfilled = a report referencing this assignment exists. '
...
COMMENT ON COLUMN reports.trust_status IS
    ...
    'Promotion to trusted is a human/admin verdict (Admin UI / SetReportTrust), '
