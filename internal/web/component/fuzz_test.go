...
func FuzzIsValidAvatarURL(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, url string) {
		...
		if strings.HasPrefix(url, "/") && result {
			// Decode URL to check actual path traversal patterns
			decoded, err := neturl.PathUnescape(url)
			if err == nil && strings.Contains(decoded, "..") && !isLegitimateDotsInFilename(decoded) {
				// Check if it's actually path traversal (not just dots in filename)
				parts := strings.Split(decoded, "/")
				for _, part := range parts {
					if part == ".." {
						t.Errorf("path traversal not blocked: input=%q decoded=%q", url, decoded)
						break
					}
				}
			}
