...
func (h *Chat) Send(w http.ResponseWriter, r *http.Request) {
	...
	// Validate CSRF token if sessions are configured
	if h.sessions != nil {
		...
		if err := h.sessions.CheckCSRF(sessionID, csrfToken); err != nil {
			h.logger.Warn("CSRF validation failed", "error", err, "session", sessionID)
...
func (h *Chat) streamWithFlow(ctx context.Context, w *sse.Writer, msgID, sessionID, query string) {
	...
	finalMsg := component.MessageBubble(component.MessageProps{
		ID:          "assistant-" + msgID, // Must match MessageShell.ID for OOB replacement
