...
//
//   - item     — by title + domain. Looks up the learning item via
//     find-by-domain-and-title (NOT find-or-create — this is a read tool;
//     creating items here would silently pollute the catalog). Backs the
...
// Exactly one of {item, concept_slug, session_id} must be provided.
type AttemptHistoryInput struct {
	Item        *AttemptHistoryItemRef `json:"item,omitempty" jsonschema_description:"Look up by learning item (title+domain). Mutually exclusive with concept_slug and session_id."`
	ConceptSlug *string                `json:"concept_slug,omitempty" jsonschema_description:"Look up by concept slug. Returns attempts that explicitly observed this concept, with the matched observation attached. Mutually exclusive with item and session_id."`
	SessionID   *string                `json:"session_id,omitempty" jsonschema_description:"Look up all attempts within a specific past session, oldest first. Mutually exclusive with item and concept_slug."`
	Domain      *string                `json:"domain,omitempty" jsonschema_description:"Domain for concept_slug lookup (defaults to 'leetcode'). Ignored for item (which carries its own domain) and session_id."`
...
// AttemptHistoryItemRef identifies a learning item by title + domain.
...
type AttemptHistoryItemRef struct {
	Title  string  `json:"title" jsonschema:"required" jsonschema_description:"Learning item title (case-sensitive match)"`
	Domain *string `json:"domain,omitempty" jsonschema_description:"Item domain (defaults to 'leetcode'). Same domain semantics as record_attempt.item.domain."`
}
...
type AttemptHistoryOutput struct {
	Mode     string             `json:"mode"`             // "item", "concept", or "session"
...
func (s *Server) attemptHistory(ctx context.Context, _ *mcp.CallToolRequest, input AttemptHistoryInput) (*mcp.CallToolResult, AttemptHistoryOutput, error) {
	...
	if provided == 0 {
		return nil, AttemptHistoryOutput{}, fmt.Errorf("attempt_history: one of item, concept_slug, session_id is required")
	}
	if provided > 1 {
		return nil, AttemptHistoryOutput{}, fmt.Errorf("attempt_history: item, concept_slug, session_id are mutually exclusive")
	}
	...
	switch {
	case input.Item != nil:
		return s.attemptHistoryByItem(ctx, input.Item, limit)
...
func (s *Server) attemptHistoryByItem(ctx context.Context, ref *AttemptHistoryItemRef, limit int32) (*mcp.CallToolResult, AttemptHistoryOutput, error) {
	if ref.Title == "" {
		return nil, AttemptHistoryOutput{}, fmt.Errorf("attempt_history: item.title is required")
	}
	domain := "leetcode"
	...
	itemID, err := s.learn.FindItem(ctx, domain, ref.Title)
	if err != nil {
		...
		return nil, AttemptHistoryOutput{
			Mode:     "item",
			Resolved: false,
			Reason:   fmt.Sprintf("no item found in domain %q with title %q", domain, ref.Title),
			Attempts: []learning.Attempt{},
		}, nil
	}

	attempts, err := s.learn.AttemptsByItem(ctx, itemID, limit)
	if err != nil {
