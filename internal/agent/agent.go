// Package agent is the single source of truth for the koopa0.dev agent
...
// the only path is agent.Authorize(). task.Store and artifact.Store mutation
// methods accept Authorized as a parameter, so bypassing the check fails at
// compile time, not at runtime.
//
// See docs/architecture/coordination-layer-target.md §4, §7, §10 for the
...
type Schedule struct {
	...
	// agent_schedule_runs.schedule_name ("<agent>:<schedule>").
	Name string
