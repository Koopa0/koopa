...
func TestHandler_SubmitFeedback_InvalidUUID(t *testing.T) {
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, "/api/admin/feed-entries/PLACEHOLDER/feedback", body)
			req.SetPathValue("id", tt.id)
			w := httptest.NewRecorder()
...
func TestHandler_SubmitFeedback_InvalidFeedbackValue(t *testing.T) {
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			req := httptest.NewRequest(http.MethodPost, "/api/admin/feed-entries/PLACEHOLDER/feedback", strings.NewReader(tt.body))
