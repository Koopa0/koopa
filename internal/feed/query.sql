...
SELECT sqlc.arg('feed_id')::uuid, UNNEST(sqlc.arg('topic_ids')::uuid[])
ON CONFLICT (feed_id, topic_id) DO NOTHING;

-- name: DeleteFeedTopics :exec
...
DELETE FROM feed_topics WHERE feed_id = sqlc.arg('feed_id')::uuid;
