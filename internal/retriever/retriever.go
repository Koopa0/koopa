...
// Retriever bridges knowledge.VectorStore to Genkit ai.Retriever interface.
...
// New creates a new Retriever with the given vector store.
// For session-specific retrieval, use NewWithSession instead.
func New(store knowledge.VectorStore) *Retriever {
	return &Retriever{
		store:     store,
		sessionID: "",
	}
...
func NewWithSession(store knowledge.VectorStore, sessionID string) *Retriever {
	return &Retriever{
		store:     store,
		sessionID: sessionID,
	}
...
// DefineDocument defines a Genkit retriever for documents (Notion, manual notes, etc.).
// It searches only documents (source_type="notion" or "manual") from the knowledge store.
//
// This is a placeholder for future implementation when document syncing is added.
func (r *Retriever) DefineDocument(g *genkit.Genkit, name string) ai.Retriever {
	return genkit.DefineRetriever(
		...
		func(ctx context.Context, req *ai.RetrieverRequest) (*ai.RetrieverResponse, error) {
			...
			// Note: chromem-go doesn't support OR/NOT filters yet
			// We search all and filter manually
			results, err := r.store.Search(ctx, queryText,
				knowledge.WithTopK(topK*2), // Request more to compensate for filtering
			)
			if err != nil {
				return nil, err
			}

			// Filter out conversations manually
			var filtered []knowledge.Result
			for _, result := range results {
				if sourceType := result.Document.Metadata["source_type"]; sourceType != "conversation" {
			...
			docs := convertToGenkitDocuments(filtered)
