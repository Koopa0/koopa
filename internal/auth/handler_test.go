...
func TestHashToken_Properties(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unicode", token: "用戶令牌"},                          //#nosec G101 -- test fixture, not a real credential
		{name: "sql injection", token: "'; DROP TABLE users; --"}, //#nosec G101 -- test fixture, not a real credential
		{name: "xss", token: `<script>alert(1)</script>`},         //#nosec G101 -- test fixture, not a real credential
...
func BenchmarkHashToken(b *testing.B) {
	b.ReportAllocs()
	const token = "dGVzdC10b2tlbi12YWx1ZS1mb3ItYmVuY2htYXJr" //#nosec G101 -- benchmark fixture, not a real credential
	for b.Loop() {
		hashToken(token)
	}
}
