...
// newTestHandler builds a Handler with a nil store + nil pool and a discard
// logger — every caller in this file passes nil, because these tests exercise
// handler-level decoding/validation before any store call. Mutation paths
// live in integration tests.
func newTestHandler() *Handler {
	return NewHandler(nil, "https://koopa0.dev", slog.New(slog.NewTextHandler(io.Discard, nil)))
}
...
// for status and visibility when they are omitted.
...
// check the handler does not reject it for missing status.
//
// Actually: the handler only rejects if slug, title, or type are empty.
// Status defaults are applied silently. We confirm that by
...
func TestHandler_Create_DefaultsApplied(t *testing.T) {
	...
	// Body with all required fields but no status/visibility.
...
func TestRegression_ErrNotFound_SentinelIdentity(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			if diff := cmp.Diff(tt.want, errors.Is(tt.err, tt.target)); diff != "" {
