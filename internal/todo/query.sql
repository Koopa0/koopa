...
VALUES (@title, @state::todo_state, @due, @project_id, @energy, @priority, @description, @assignee, @created_by)
...
-- name: UpsertTodoByExternalRef :one
...
VALUES (@title, @state::todo_state, @due, @project_id, @external_provider, @external_ref,
        CASE WHEN @state::todo_state = 'done' THEN now() ELSE NULL END,
        @energy, @priority, @recur_interval, @recur_unit, @description, @assignee)
...
-- name: TodoExternalRefsByProvider :many
-- List all external_ref values for todos synced from a specific provider.
SELECT external_ref FROM todos
WHERE external_provider = @external_provider AND external_ref IS NOT NULL
ORDER BY title;

-- name: ArchiveTodoByExternalRef :execrows
...
WHERE external_provider = @external_provider AND external_ref = @external_ref
  AND state != 'done'
  AND (recur_interval IS NULL OR recur_interval <= 0);

-- name: ArchiveOrphanTodosByProvider :execrows
-- Mark non-recurring todos from a provider as done if their external_ref is not in the active set.
UPDATE todos SET state = 'done', completed_at = COALESCE(completed_at, now()), updated_at = now()
WHERE external_provider = @external_provider
  AND external_ref IS NOT NULL
  AND external_ref != ALL(@active_ids::text[])
  AND state != 'done'
  AND (recur_interval IS NULL OR recur_interval <= 0);
...
-- name: TodoByExternalRef :one
-- Get a todo by its (external_provider, external_ref) pair.
SELECT id, title, state, due, project_id, external_provider, external_ref,
       completed_at, energy, priority, recur_interval, recur_unit,
       description, assignee, created_by, created_at, updated_at
FROM todos WHERE external_provider = @external_provider AND external_ref = @external_ref;
