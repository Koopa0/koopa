...
func TestGroupSessions_Adversarial(t *testing.T) {
	...
	t.Run("exactly 29m59s gap — same session", func(t *testing.T) {
		t.Parallel()
		events := []Event{
			{Timestamp: base.Add(29*time.Minute + 59*time.Second), Source: "github"},
			{Timestamp: base, Source: "github"},
		}
	...
	t.Run("empty project string treated same as nil — excluded from projects", func(t *testing.T) {
		...
		empty := ""
		events := []Event{
			{Timestamp: base, Source: "github", Project: &empty},
		}
		got := GroupSessions(events)
		if len(got) != 1 {
			t.Fatalf("GroupSessions() = %d sessions, want 1", len(got))
	...
	t.Run("duplicate source in multiple events — deduplicated in Sources", func(t *testing.T) {
		t.Parallel()
		events := []Event{
			{Timestamp: base.Add(10 * time.Minute), Source: "github"},
			{Timestamp: base.Add(5 * time.Minute), Source: "github"},
			{Timestamp: base, Source: "github"},
		}
		got := GroupSessions(events)
		if len(got) != 1 {
	...
	t.Run("sources sorted alphabetically", func(t *testing.T) {
		t.Parallel()
		events := []Event{
			{Timestamp: base.Add(5 * time.Minute), Source: "obsidian"},
			{Timestamp: base, Source: "github"},
		}
		got := GroupSessions(events)
		if len(got) != 1 {
	...
	t.Run("projects sorted alphabetically", func(t *testing.T) {
		...
		projZ := "zeta"
		events := []Event{
			{Timestamp: base.Add(5 * time.Minute), Source: "github", Project: &projZ},
			{Timestamp: base, Source: "github", Project: &projA},
		}
		got := GroupSessions(events)
		if len(got) != 1 {
			t.Fatalf("GroupSessions() = %d sessions, want 1", len(got))
	...
	t.Run("session duration truncated to minute precision", func(t *testing.T) {
		...
		events := []Event{
			{Timestamp: base.Add(15*time.Minute + 30*time.Second), Source: "github"},
			{Timestamp: base, Source: "github"},
		}
		got := GroupSessions(events)
		if len(got) != 1 {
	...
	t.Run("result is always ordered newest first", func(t *testing.T) {
		...
		s3 := base
		events := []Event{
			{Timestamp: s1, Source: "github"},
			// gap > 30m before s2
			{Timestamp: s2, Source: "github"},
			// gap > 30m before s3
			{Timestamp: s3, Source: "github"},
		}
		got := GroupSessions(events)
		for i := 1; i < len(got); i++ {
	...
	t.Run("unicode source name stored verbatim", func(t *testing.T) {
		t.Parallel()
		events := []Event{
			{Timestamp: base, Source: "obsidian-繁體中文"},
		}
		got := GroupSessions(events)
		if len(got) != 1 {
			t.Fatalf("GroupSessions() = %d sessions, want 1", len(got))
		}
		if !slices.Contains(got[0].Sources, "obsidian-繁體中文") {
			t.Errorf("GroupSessions() Sources = %v, want to contain %q", got[0].Sources, "obsidian-繁體中文")
		}
	})
}
...
func TestGroupSessions_SessionCount_Full(t *testing.T) {
	...
	events := []Event{
		{Timestamp: base.Add(11*time.Hour + 20*time.Minute), Source: "obsidian", Project: &projBlog},
		{Timestamp: base.Add(11 * time.Hour), Source: "github", Project: &projBlog},
		{Timestamp: base.Add(10 * time.Minute), Source: "github", Project: &projBlog},
		{Timestamp: base, Source: "obsidian", Project: &projBlog},
	}
	...
	want := []Session{
		{
			...
			Duration:   "20m0s",
			EventCount: 2,
			Sources:    []string{"github", "obsidian"},
...
func FuzzGroupSessions(f *testing.F) {
	...
	f.Add("", int64(0))
	f.Add("obsidian", int64(1800))       // exactly 30 min
	f.Add("github", int64(1799))         // 29m59s
	f.Add("unicode-中文", int64(-3600))    // negative offset
	f.Add("'; DROP TABLE --", int64(60)) // SQL injection in source

	f.Fuzz(func(t *testing.T, source string, offsetSec int64) {
		project := "project"
		events := []Event{
			{
...
func BenchmarkGroupSessions(b *testing.B) {
	...
	for i := range events {
		...
		events[i] = Event{
			...
			Source:    []string{"github", "obsidian", "rss"}[i%3],
		}
	}
