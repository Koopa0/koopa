...
> 讀側分析統一在 `learning_read`（涵蓋 overview / next_target / attempts / session_progress）。
> `brief` 與 `learning_read` 是 READ-ONLY。MCP 上沒有 FSRS / review 工具、沒有 A2A 協調
> （directive / report）、也沒有 agent_notes。
