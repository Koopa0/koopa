...
|------|------|------------|
| `start_session` | 開始學習 | `domain` (seeded: `leetcode` / `japanese` / `go` / `system-design` / `reading`; 其他可透過 `propose_commitment(type=learning_domain)` 新增), `mode` (practice/retrieval/mixed/review/reading), `daily_plan_item_id?` |
