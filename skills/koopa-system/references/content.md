...
> MCP-v3 後，content 發布生命週期（create / update / review / publish / archive / list / read）
> 已全部移到 admin HTTP（人類操作），不在 MCP surface。跨 agent 協調三元組與 report-lane 也已移除。
> 你的 MCP 寫入能力收斂為 **Zettelkasten note**（`create_note` / `update_note`）。
...
## Content Pipeline (post-contraction)
