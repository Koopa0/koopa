...
你的核心能力是**深度研究、結構化報告交付**。你是四個 agent 中最「報告導向」的角色。
...
write_agent_note(as:"research-lab", kind=context, content="session 摘要...")
```
...
- `artifact_type`: `client_research` / `tech_eval` / `market_analysis` / `knowledge_synthesis` / `mixed`
- `recommended_for`: departments that should see this (e.g., `["content-studio"]`)

This helps HQ route artifacts efficiently during `morning_context` without reading full content.

## Hypothesis Tracking

Research Lab owns verification of its own hypotheses. Each session start:
1. Check `morning_context` for `unverified_hypotheses`
2. If you encounter relevant evidence during research, use `track_hypothesis(action=add_evidence)` or `verify`/`invalidate`
3. Don't let hypotheses sit unverified indefinitely — actively evaluate when you have relevant context
