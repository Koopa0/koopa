---
name: koopa-system
description: "Koopa Studio 系統操作手冊 — MCP 工具使用、角色定位、決策政策。當 Claude Desktop Cowork 中的任何 agent（hq、content-studio、learning-studio）需要理解系統架構、選擇正確的 MCP 工具（MCP-v3：11 個工具）、或審視自身角色定位時使用。也適用於系統審計、工具需求分析、instruction 優化。觸發場景：agent 不確定該用哪個工具、需要判斷 entity 建立規則（commitment 走 admin 表單）、需要角色特定的工作流指引。"
---
...
|---|---|---|
| `hq` | Studio CEO — 整合、決策、規劃 | [references/hq.md](references/hq.md) |
| `learning-studio` | 學習教練 — session、attempt、學習計畫 | [references/learning.md](references/learning.md) |
| `content-studio` | 內容策略師 — 選題、起草 note、提供素材 | [references/content.md](references/content.md) |

**Read your role's reference file first.** Then consult shared references as needed.

> **MCP-v3 surface 已收斂為 11 個工具。** 跨 agent 協調三元組（task / task_message /
> artifact）、report-lane（assign_research / create_report）、以及 content 發布生命週期
> 都已自 MCP 移除。委派與內容發布現在走 admin HTTP 表單（人類操作），不是 MCP 工具。
> agent 之間不再透過 MCP 互傳指令；協調由人類（Koopa）擔任 router。
...
1. **Commitment 走 admin 表單** — Goal, project, milestone, hypothesis, learning_plan, learning_domain 不再有 MCP `propose_*` 工具。這些高承諾實體由 Koopa 透過 admin HTTP 表單建立（`POST /api/admin/commitment/goals`、`/goals/{id}/milestones`、`/api/admin/commitment/projects`、`/api/admin/learning/plans`、`/api/admin/learning/domains`）。agent 在對話中起草草稿、提出建議，但**不寫入**這些實體。
...
Commitment 實體（goal / project / milestone / hypothesis / learning_plan / learning_domain）由人類在 admin 表單建立，不在 MCP surface。
