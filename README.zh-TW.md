...
| 身分 | 執行環境 | 角色 |
|---|---|---|
| `planner` | Claude Cowork | 晨間 briefing、候選日計畫、inbox capture、搜尋、note 共同作者 |
| `learning-studio` | Claude Cowork | 學習教練 — session、attempt、curriculum |
...
任何 agent 都能透過 MCP 用 `search_knowledge` 查詢已發佈的 content 與 Zettelkasten note，背後是 hybrid 檢索：PostgreSQL 全文搜尋（tsvector 配 websearch 語法、GIN 索引）加上 pgvector 語意搜尋（HNSW、cosine），每個 corpus 各自以 reciprocal-rank fusion 融合。背景的 reconciler 會在新的 content 與 note 落地後自動產生 embedding（`gemini-embedding-2`），語意側保持最新而不碰任何 request path；沒有設定 `GEMINI_API_KEY` 時，搜尋以 FTS-only 運作。
...
| Embedding     | `gemini-embedding-2`（1536d Matryoshka）；背景 reconciler 維持 content 與 note 的 embedding 最新 |
...
| AI 協作       | Claude（Cowork + Code）、Codex CLI、MCP（11 個工作流工具）                     |
