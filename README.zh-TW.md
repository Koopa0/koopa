...
已發布的 content 與 Zettelkasten note 可被任何 agent 透過 MCP 用 `search_knowledge` 查詢。它跑**混合檢索** — PostgreSQL 全文搜尋（tsvector 配 websearch 語法，GIN 索引）**加**pgvector 語意搜尋（1536 維 `gemini-embedding-2-preview`，Matryoshka 截斷，HNSW 索引）— 用 reciprocal rank fusion 合併結果。Agent 同時找得到關鍵字匹配的內容和語意匹配的內容，不需要先選策略。
