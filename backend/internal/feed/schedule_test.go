...
func TestValidSchedule_Adversarial(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "leading whitespace", input: " hourly_4", want: false},
...
func decodeErrBody(t *testing.T, body *strings.Reader) api.ErrorBody {
	t.Helper()
...
func TestFeedHandler_Create(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "hourly_4 schedule is valid",
			body:       `{"url":"https://example.com/feed.xml","name":"Example","schedule":"hourly_4"}`,
			stub:       &stubFeedStore{created: fixtureFeed()},
			wantStatus: http.StatusCreated,
		},
		{
